...
|---|---|---|
| **A · Smart conversation export** | Markdown / PDF / JSON / **long PNG** export, timestamp switch, privacy redaction (auto-masking of phone numbers / emails / ID numbers / bank cards), multi-session batch ZIP; PDFs containing Chinese are rasterized client-side into **print-free multi-page PDFs** (no `window.print()` dialog freeze) | `enableExport` |
| **B · Context handoff summary** | One-click ≤500-char four-section handoff summary, editable, copy to clipboard, save as template; import a summary as the first `system` message of a new conversation for cross-conversation context inheritance | `enableHandoff` |
...
- **Batch export**: select multiple sessions in the history list and click "Batch export" (or the `export-batch` command) to get a ZIP (up to 100 sessions per run, auto-deduplicated).
...
- **Generate**: click the "Handoff summary" button in the conversation header (or the `handoff` command); the plugin calls the DeepSeek API with a fixed four-section prompt: core conclusions / resolved problems / key background / open to-dos and unresolved questions (≤500 chars).
...
- **Cost reports**: a dedicated view shows daily/weekly token usage, cost estimates (resolved live by the pricing engine), daily/monthly budget progress bars (yellow at 80%, red at 100%), pricing source (official live / built-in snapshot) and the peak/off-peak plan, plus manual pricing refresh and the amount saved by peak-aware scheduling. The `usage` command prints this month's text report inside the panel.
...
| `dsh-conv-export` | Long-PNG raster export, print-free multi-page PDF (foreignObject → canvas → JPEG → zero-dependency PDF assembly), image data-URL inlining | `src/client/raster.ts`, `src/modules/export/` (kind:'raster' payloads) |
