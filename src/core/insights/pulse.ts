...
/** 严重度排序权重。 */
const SEVERITY_ORDER: ReadonlyMap<InsightSeverity, number> = new Map([
  ['critical', 0],
  ['watch', 1],
  ['info', 2],
])
 ...
 * @param now 合成时间戳。
 */
export function composePulse(providers: readonly InsightProvider[], now: number): PulseReport {
...
export function blindSpotInsights(report: {
  blindSpots: ReadonlyArray<{ anchor: string; searches: number; advice: string }>
...
/** 人话摘要。 */
function summarize(cards: readonly InsightCard[]): string {
  if (cards.length === 0) return '一切正常：没有值得主动告知的信号'
  const critical = cards.filter((card) => card.severity === 'critical').length
  const watch = cards.filter((card) => card.severity === 'watch').length
  const parts: string[] = [`${cards.length} 条洞察`]
  if (critical > 0) parts.push(`${critical} 条需立即关注`)
  if (watch > 0) parts.push(`${watch} 条建议留意`)
  return parts.join('，')
}

/** 到期意图信号 → 洞察卡片（轴线 20 的主动化出口）。 */
export function dueIntentionInsights(due: ReadonlyArray<{ text: string; overdueDays: number }>): readonly InsightCard[] {
  if (due.length === 0) return []
  const top = due[0]
  const severity: InsightSeverity = top.overdueDays >= 14 ? 'critical' : 'watch'
  return [
    {
      category: 'intention',
      severity,
...
export function dueReviewInsights(due: ReadonlyArray<{ fresh: boolean }>): readonly InsightCard[] {
  if (due.length === 0) return []
...
export function forgettingForecastInsights(stats: {
  ...
  if (stats.criticalCount > 0) {
    const warningPart =
      stats.warningCount > 0 ? `，另有 ${stats.warningCount} 条正在滑落` : ''
    return [
      {
        category: 'forecast',
        severity: 'critical',
