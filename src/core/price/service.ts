...
function sanitizePriceTable(raw: unknown): PriceTable {
  ...
  for (const [model, value] of Object.entries(raw as Record<string, unknown>)) {
    ...
    if (typeof entry.inputCacheHit !== 'number' || typeof entry.inputMiss !== 'number' || typeof entry.output !== 'number') continue
...
function sanitizePeakWindow(w: unknown): [number, number] | undefined {
  if (!Array.isArray(w) || w.length !== 2) return undefined
  const [start, end] = w
  if (typeof start !== 'number' || typeof end !== 'number') return undefined
  if (!Number.isFinite(start) || !Number.isFinite(end)) return undefined
  if (start < 0 || start > 24 || end < 0 || end > 24) return undefined
...
export function sanitizePriceSheet(raw: unknown): PriceSheet | undefined {
  ...
  if (typeof entry.scheduled === 'object' && entry.scheduled !== null && !Array.isArray(entry.scheduled)) {
    ...
    if (effectiveOk && Object.keys(peak).length > 0 && Object.keys(offPeak).length > 0) {
      const scheduled: ScheduledPricing = { effective: sched.effective as string, offPeak, peak }
