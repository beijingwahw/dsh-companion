...
export function analyzeBlindSpots(
  ...
  const anchors = new Map<string, AnchorAgg>()
  for (const record of records) {
    for (const term of tokenizeTerms(record.query)) {
      if (term.length < ANCHOR_MIN_LENGTH) continue
  ...
  for (const [anchor, agg] of anchors) {
    if (agg.searches < MIN_SEARCHES) continue
...
export function sanitizeMissRecord(raw: unknown): MissRecord | undefined {
  ...
  if (query.length === 0 || count < 1 || lastAt <= 0) return undefined
  // firstAt 可缺省（旧版记录）：仅在合法时透传，缺省交给聚合层回退 lastAt。
  const firstAt = typeof record.firstAt === 'number' ? record.firstAt : undefined
  if (firstAt !== undefined && (firstAt <= 0 || firstAt > lastAt)) return undefined
...
function adviceFor(anchor: string, searches: number, ageDays: number): string {
...
/** 人话摘要。 */
function summarize(blindSpots: readonly RetrievalBlindSpot[], totalMisses: number): string {
