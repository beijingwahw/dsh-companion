...
/** 全部特征（展示与诊断顺序）。 */
export const RANKER_FEATURES: readonly RankerFeatureName[] = [
  'lexicalRank',
  'semanticRank',
  'recency',
  'feedback',
...
/** 空模型（冷启动）。 */
export function emptyRankerModel(): RankerModel {
  return { z: {}, n: {}, updates: 0, clicks: 0, examples: 0, trainedAt: 0 }
}

/** 从损坏/旧格式记录恢复的收窄辅助：非法记录静默丢弃。 */
export function sanitizeRankerModel(raw: unknown): RankerModel | undefined {
  if (typeof raw !== 'object' || raw === null) return undefined
  const record = raw as Record<string, unknown>
  const accumulator = (value: unknown): Record<string, number> => {
  ...
  if (model.updates === 0 && model.clicks === 0 && model.examples === 0) return undefined
  return model
}
 ...
 */
export function buildRankerFeatures(input: RankerFeatureInput): RankerFeatures {
  ...
  return {
    lexicalRank:
      input.lexicalRank !== undefined && input.lexicalRank > 0
        ? 1 / (RANK_FEATURE_K + input.lexicalRank)
        : 0,
    semanticRank:
      input.semanticRank !== undefined && input.semanticRank > 0
 ...
 */
export function effectiveWeights(model: RankerModel): Record<RankerFeatureName, number> {
  const weights = {} as Record<RankerFeatureName, number>
...
export function predictClickProbability(model: RankerModel, features: RankerFeatures): number {
  const weights = effectiveWeights(model)
  let dot = 0
  for (const feature of RANKER_FEATURES) {
    const value = features[feature]
    if (value !== 0) dot += weights[feature] * value
  }
  return sigmoid(dot)
}
...
export function updateRanker(
  ...
  const z: Record<string, number> = { ...model.z }
  const n: Record<string, number> = { ...model.n }
  const weights = effectiveWeights(model)
  for (const feature of RANKER_FEATURES) {
...
export function rankerWarmup(model: RankerModel): number {
  if (model.updates <= 0) return 0
 ...
 */
export function learnedMultiplier(model: RankerModel, features: RankerFeatures): number {
...
export function rankerDiagnostics(model: RankerModel): {
  updates: number
  clicks: number
  examples: number
  warmup: number
  trainedAt: number
  features: ReadonlyArray<{ name: RankerFeatureName; weight: number }>
} {
  const weights = effectiveWeights(model)
  return {
