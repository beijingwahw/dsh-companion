 ...
 */
export function expandQuery(queryText: string, corpus: QueryCorpusView): ExpandedQuery {
  const terms = [...new Set(tokenize(queryText))]
  if (terms.length === 0) return { queryText, extraTerms: [], notes: [] }
  const querySet = new Set(terms)
  const added = new Set<string>()
  const notes: QueryExpansionNote[] = []
  // 1) 拼写纠错：优先级最高（拼错的词在词法通道是纯损失）。
  for (const term of terms) {
    if (notes.length >= MAX_EXPANSION_TERMS) break
    if (corpus.termDf.get(term) !== undefined) continue
    const correction = nearestVocabularyTerm(term, corpus.termDf)
    if (correction === undefined || querySet.has(correction) || added.has(correction)) continue
  ...
  if (notes.length < MAX_EXPANSION_TERMS) {
    const candidates: Array<{ note: QueryExpansionNote; salience: number }> = []
    for (const term of terms) {
      const expansion = strongestCooccurrence(term, corpus, querySet, added)
      if (expansion !== undefined) candidates.push(expansion)
    }
  ...
  if (notes.length === 0) return { queryText, extraTerms: [], notes: [] }
 ...
 */
function nearestVocabularyTerm(
  term: string,
  termDf: ReadonlyMap<string, number>,
): string | undefined {
  if (term.length < CORRECTION_MIN_TERM_LENGTH) return undefined
  ...
  for (const [candidate, df] of termDf) {
    if (df < CORRECTION_MIN_DF) continue
    if (Math.abs(candidate.length - term.length) > CORRECTION_MAX_LENGTH_DELTA) continue
 ...
 */
function strongestCooccurrence(
  term: string,
  corpus: QueryCorpusView,
  querySet: ReadonlySet<string>,
  added: ReadonlySet<string>,
): { note: QueryExpansionNote; salience: number } | undefined {
  const df = corpus.termDf.get(term) ?? 0
  if (df < COOCCURRENCE_MIN_DOCS) return undefined
  ...
  let best: { note: QueryExpansionNote; salience: number } | undefined
  for (const [other, coDocs] of coCounts) {
    if (coDocs < COOCCURRENCE_MIN_DOCS) continue
    if (querySet.has(other) || added.has(other)) continue
