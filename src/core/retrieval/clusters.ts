 ...
 */
import { docGramVector, sparseCosine, type IndexedDoc } from './engine.js'
...
export function clusterSessions(
  ...
  const labeled = clusters.map((cluster): SessionCluster & { score: number } => {
    const intraDf = new Map<string, number>()
