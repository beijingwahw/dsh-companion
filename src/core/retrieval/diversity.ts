...
export function mmrSelect<T>(
  ...
  if (take === pool.length && pool.length < MMR_MIN_POOL) return [...pool]
  // 首选：相关度最高者（无已选集合，多样性项为 0）。
  const remaining = [...pool]
  const selected: T[] = []
  let firstIndex = 0
  for (let i = 1; i < remaining.length; i += 1) {
    if (relevanceOf(remaining[i]) > relevanceOf(remaining[firstIndex])) firstIndex = i
  }
 ...
 */
export function diversityPoolSize(limit: number, totalHits: number): number {
