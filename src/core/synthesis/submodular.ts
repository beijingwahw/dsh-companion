...
export interface SubmodularResult {
  ...
  readonly selected: ReadonlyArray<{ index: number; text: string }>
  /** 选后集合的问题方面覆盖率（[0,1]）。 */
 ...
 * @param options 约束与权重。
 */
export function selectSubmodular<T extends SelectableChunk>(
  candidates: readonly T[],
  question: string,
  options: SubmodularOptions,
): SubmodularResult {
  ...
  for (const aspect of aspects.keys()) covered.set(aspect, 0)

  /** 块的边际增益（相对当前 covered 状态；O(方面数)）。 */
  const marginalGain = (position: number): number => {
    ...
    for (const [aspect, cov] of coverageVectors[position]) {
      ...
      if (cov > prev) gain += coverageWeight * aspectWeightOf(aspects, aspect) * (cov - prev)
    }
    return gain
  }
  ...
  let totalChars = 0
  // 队列项：{ position, staleGain, priority }——stale 为入队时的增益。
  interface QueueItem {
    position: number
    staleGain: number
    priority: number
  }
  let queue: QueueItem[] = usable.map((_, position) => {
  ...
  const sessionCapped = new Set<number>()
  while (selected.length < options.maxChunks && queue.length > 0) {
    ...
    if (sessionFull) sessionCapped.add(head.position)
    ...
    if (queue.length === 1 || freshPriority >= queue[1].priority) {
      ...
      for (const [aspect, cov] of coverageVectors[head.position]) {
        if (cov > (covered.get(aspect) ?? 0)) covered.set(aspect, cov)
      }
    ...
    const reinserted: QueueItem = { position: head.position, staleGain: freshGain, priority: freshPriority }
    queue = queue.slice(1)
  ...
  if (remaining >= tailFloor && selected.length < options.maxChunks) {
    ...
    for (let position = 0; position < usable.length; position += 1) {
      if (sessionCapped.has(position)) continue
      if (selected.some((item) => item.index === usable[position].index)) continue
      const chunk = usable[position].chunk
      if (chunk.text.length < tailFloor) continue
      const used = perSession.get(chunk.sessionId) ?? 0
      if (used >= options.perSessionCap) continue
      const gain = marginalGain(position)
      evaluations += 1
      if (gain > bestGain) {
        bestGain = gain
 ...
 * @param texts 证据文本列表。
 */
export function evidenceCoverage(question: string, texts: readonly string[]): number {
...
function coverageScore(covered: ReadonlyMap<string, number>, aspects: ReadonlyMap<string, { weight: number; maxTf: number }>): number {
  let totalWeight = 0
  let coveredWeight = 0
  for (const [aspect, info] of aspects) {
    if (info.maxTf <= 0) continue
...
function buildAspects(
...
): Map<string, { weight: number; maxTf: number }> {
  const aspects = new Map<string, { weight: number; maxTf: number }>()
  ...
  for (const text of texts) {
    ...
    for (const aspect of aspects.keys()) {
      ...
      if (info !== undefined && count > info.maxTf) info.maxTf = count
    }
  }
  for (const [aspect, info] of aspects) {
...
function coverageVectorOf(
  text: string,
  aspects: ReadonlyMap<string, { weight: number; maxTf: number }>,
): Map<string, number> {
  const vector = new Map<string, number>()
  if (aspects.size === 0) return vector
  const tf = new Map<string, number>()
  for (const token of tokenize(text)) {
    if (aspects.has(token)) tf.set(token, (tf.get(token) ?? 0) + 1)
  }
  for (const [aspect, count] of tf) {
    const info = aspects.get(aspect)
    if (info !== undefined && info.maxTf > 0) vector.set(aspect, count / info.maxTf)
  }
  return vector
}

/** 方面权重读取（缺省 1 防御）。 */
function aspectWeightOf(
  aspects: ReadonlyMap<string, { weight: number; maxTf: number }>,
  aspect: string,
): number {
...
function insertSorted(queue: Array<{ position: number; staleGain: number; priority: number }>, item: { position: number; staleGain: number; priority: number }): void {
  let low = 0
  let high = queue.length
  while (low < high) {
    const mid = (low + high) >>> 1
    if (queue[mid].priority >= item.priority) low = mid + 1
    else high = mid
  }
