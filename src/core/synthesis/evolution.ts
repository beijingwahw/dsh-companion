...
interface ExtractedClaim {
  ...
  /** 版本号（如 v2.10；可能缺省——纯数值声明）。 */
  readonly version?: string
  /** 数值（如 8、0.5；可能缺省——纯版本声明）。 */
 ...
 */
export function analyzeEvolution(inputs: readonly EvolutionInput[]): EvolutionReport {
  if (inputs.length < 2) {
    return { events: [], summary: '证据不足：跨会话演化分析需要至少两个时间点的证据' }
  }
  const ordered = [...inputs].sort((a, b) => a.createdAt - b.createdAt)
  // 逐块提取声明（含来源回指）。
  const claims = ordered.flatMap((input) =>
    extractClaims(input.text).map((claim) => ({ claim, source: input })),
  )
  if (claims.length === 0) {
    return { events: [], summary: '未在证据中检测到可比较的版本号或数值声明' }
  }
  // 按主题锚分组；组内两两比较生成演化事件（时间早者为旧信念）。
  const byAnchor = new Map<string, Array<{ claim: ExtractedClaim; source: EvolutionInput }>>()
  for (const entry of claims) {
    const group = byAnchor.get(entry.claim.anchor)
    if (group === undefined) byAnchor.set(entry.claim.anchor, [entry])
...
function extractClaims(text: string): ExtractedClaim[] {
  ...
  const versionSpans: Array<[number, number]> = []
  ...
  for (const pattern of VERSION_PATTERNS) {
    for (const match of text.matchAll(pattern)) {
      ...
      if (anchor !== undefined) claims.push({ anchor, version })
    }
  }
  ...
  for (const match of text.matchAll(NUMBER_PATTERN)) {
    ...
    const anchor = nearestAnchor(text, start)
    if (anchor !== undefined) claims.push({ anchor, value: match[1] })
  }
 ...
 */
function nearestAnchor(text: string, position: number): string | undefined {
  const prefix = text.slice(Math.max(0, position - 40), position)
  const tokens = tokenize(prefix)
  for (let i = tokens.length - 1; i >= 0; i -= 1) {
    const token = tokens[i]
    if (token.length < ANCHOR_MIN_TOKEN_LENGTH) continue
...
function compareClaims(
  anchor: string,
  a: { claim: ExtractedClaim; source: EvolutionInput },
  b: { claim: ExtractedClaim; source: EvolutionInput },
): EvolutionEvent | undefined {
  if (anchor.length < ANCHOR_MIN_SHARED_LENGTH) return undefined
  ...
  if (a.source.sessionId === b.source.sessionId) return undefined
  const [older, newer] =
    a.source.createdAt <= b.source.createdAt
      ? [a, b]
      : [b, a]
  const kind: EvolutionKind =
    older.claim.version !== undefined && newer.claim.version !== undefined
 ...
 */
export function compareVersions(a: string, b: string): EvolutionKind {
  ...
  const sa = parse(a)
  const sb = parse(b)
  if (sa.length === 0 || sb.length === 0) return 'change'
  const length = Math.max(sa.length, sb.length)
  for (let i = 0; i < length; i += 1) {
    const da = sa[i] ?? 0
    const db = sb[i] ?? 0
    // a 为旧值、b 为新值：旧 < 新 → upgrade；旧 > 新 → downgrade。
    if (da < db) return 'upgrade'
    if (da > db) return 'downgrade'
  }
  return 'change'
}

/** 人话摘要：事件计数 + 最新一次演化的方向与值。 */
function summarize(events: readonly EvolutionEvent[]): string {
  if (events.length === 0) {
    return '未检测到信念变化：各时间点的版本号与数值陈述一致'
  }
  const latest = events[events.length - 1]
  const direction =
    latest.kind === 'upgrade' ? '升级' : latest.kind === 'downgrade' ? '回退' : '变化'
  const heads = events
    .slice(0, 3)
