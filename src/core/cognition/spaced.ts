 ...
 */
export function initialDueAt(episodeCreatedAt: number): number {
  return episodeCreatedAt + INTERVALS_DAYS[0] * DAY_MS
}

/** 已复习片段的下一次到期时间：最近复习时间 + 档位间隔 × 排程节律。 */
export function nextDueAt(state: ReviewState): number {
...
export function reviewStrength(state: ReviewState | undefined): number {
...
export function sanitizeReviewState(raw: unknown): ReviewState | undefined {
  if (typeof raw !== 'object' || raw === null) return undefined
  const state = raw as Record<string, unknown>
  if (typeof state.episodeId !== 'string' || state.episodeId.length === 0) return undefined
  if (typeof state.stage !== 'number' || !Number.isInteger(state.stage) || state.stage < 0) {
    return undefined
  }
  if (state.stage >= INTERVALS_DAYS.length) return undefined
  const last = state.lastReviewedAt
  if (last !== null && typeof last !== 'number') return undefined
