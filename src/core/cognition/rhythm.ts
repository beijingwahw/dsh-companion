...
/** 空档案（冷启动：标准曲线 + 零样本）。 */
export function emptyRhythm(): RhythmProfile {
  return { remembered: 0, forgotten: 0, ease: 1, updatedAt: null }
}

/** 命中率（0..1；零样本返回 0，由调用方在展示层规避）。 */
export function rhythmHitRate(profile: RhythmProfile): number {
 ...
 */
export function adaptiveIntervalDays(baseDays: number, ease: number): number {
 ...
 */
export function projectedLadder(
  ease: number,
): ReadonlyArray<{ stage: number; baseDays: number; adaptedDays: number }> {
...
/** 节律档案人话摘要（三向判定 + 命中率播报）。 */
export function rhythmSummary(profile: RhythmProfile): string {
  const total = profile.remembered + profile.forgotten
  if (total === 0) {
...
export function sanitizeRhythmProfile(raw: unknown): RhythmProfile | undefined {
  if (typeof raw !== 'object' || raw === null) return undefined
  const profile = raw as Record<string, unknown>
  if (
    typeof profile.remembered !== 'number' ||
  ...
  if (
    typeof profile.forgotten !== 'number' ||
    !Number.isInteger(profile.forgotten) ||
    profile.forgotten < 0
  ) {
    return undefined
  }
  if (typeof profile.ease !== 'number' || !Number.isFinite(profile.ease)) return undefined
  const updatedAt =
