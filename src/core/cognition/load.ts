 ...
 * @param cap 每日容量（缺省 8）。
 */
export function planReviewLoad(
  due: readonly DueReview[],
  retentionOf: (episodeId: string) => number | undefined,
  cap = DEFAULT_DAILY_CAP,
): LoadPlan {
  const effectiveCap = Math.max(1, Math.floor(cap))
  const scored: PlannedReview[] = due.map((review) => {
  ...
  const health: LoadHealth =
    scored.length === 0 ? 'clear' : deferredCount > 0 ? 'overload' : 'normal'
  return {
    cap: effectiveCap,
...
function summarizeLoad(
  ...
  if (totalDue === 0) return '没有到期复习——认知负荷为零'
  if (deferredCount > 0) {
