...
/** 时间标记表：正则源 → 视界天数 + 显式性。 */
const TEMPORAL_MARKERS: ReadonlyArray<{ source: RegExp; days: number; explicit: boolean }> = [
...
const INTENT_VERBS: readonly RegExp[] = [
  /试试|试一下|试个/,
  /看看|看下|看一下/,
  /研究(?:一[下番]|研究)?/,
  /优化|修复|修一下|重构|重写/,
...
const TODO_MARKERS: readonly RegExp[] = [/待办/, /\bTODO\b/i, /别忘了|别忘|记得要/, /\bremind(?:er)?\b/i]
 ...
 */
export function extractIntentions(text: string): readonly Intention[] {
  ...
  for (let index = 0; index < chunks.length; index += 2) {
    ...
    if (sentence.length < 4 || sentence.length > 300) continue
    // 问句是求助不是承诺（「怎么修复？」≠「我会修复」）。
    ...
    if (isQuestion && !hasTodo) continue
    const marker = TEMPORAL_MARKERS.find((entry) => entry.source.test(sentence))
    if (marker === undefined && !hasTodo) continue
    if (!hasTodo && !INTENT_VERBS.some((pattern) => pattern.test(sentence))) continue
    ...
    if (intentions.length >= MAX_INTENTIONS_PER_SESSION) break
  }
  return intentions
}
 ...
 */
export function dueIntentions(
  records: ReadonlyMap<string, IntentionRecord>,
  now: number,
  maxReturned = 20,
): readonly DueIntention[] {
  const due: DueIntention[] = []
 ...
 */
export function upcomingIntentions(
  records: ReadonlyMap<string, IntentionRecord>,
  now: number,
  maxReturned = 10,
): readonly DueIntention[] {
  const upcoming: DueIntention[] = []
  for (const [sessionId, record] of records) {
    for (const intention of record.intentions) {
      const dueAt = record.createdAt + intention.horizonDays * DAY_MS
      if (now >= dueAt || dueAt - now > 7 * DAY_MS) continue
      upcoming.push({
        sessionId,
        text: intention.text,
        marker: intention.marker,
...
export function sanitizeIntentionRecord(raw: unknown): IntentionRecord | undefined {
  if (typeof raw !== 'object' || raw === null) return undefined
  const record = raw as Record<string, unknown>
  if (typeof record.createdAt !== 'number' || !Number.isFinite(record.createdAt) || record.createdAt <= 0) {
    return undefined
  }
  if (!Array.isArray(record.intentions)) return undefined
  const intentions: Intention[] = []
  for (const item of record.intentions) {
    if (typeof item !== 'object' || item === null) continue
    const intention = item as Record<string, unknown>
    if (typeof intention.text !== 'string' || intention.text.length === 0) continue
    if (typeof intention.marker !== 'string') continue
    if (typeof intention.horizonDays !== 'number' || !Number.isFinite(intention.horizonDays)) continue
    if (typeof intention.explicit !== 'boolean') continue
    intentions.push({
      text: intention.text,
      marker: intention.marker,
