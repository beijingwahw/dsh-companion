...
import { extractShape, type Episode, type EpisodeShape } from './episodes.js'
 ...
 */
export function shapeSimilarity(a: EpisodeShape, b: EpisodeShape): number {
  ...
  if (constraintO === 0 && resolutionO === 0) return 0
...
function termOverlap(queryTerms: ReadonlySet<string>, episodeTerms: ReadonlySet<string>): number {
  if (queryTerms.size < MIN_ANCHOR_TERMS || episodeTerms.size < MIN_ANCHOR_TERMS) return 0.5
  let intersection = 0
  for (const term of queryTerms) if (episodeTerms.has(term)) intersection += 1
  if (intersection === 0) return 0
...
export function findAnalogies(
  queryText: string,
  episodes: ReadonlyMap<string, import('./episodes.js').EpisodeRecord>,
  ...
  if (queryShape.constraints.length === 0) {
    return {
      ...
      analogies: [],
      summary:
        '未能从描述中识别问题结构——带上问题类型关键词（如 报错/超时/依赖/版本/内存）类比会更准',
    }
  }
  const queryTerms = anchorTerms(queryText)
  const hits: AnalogyHit[] = []
  for (const [sessionId, record] of episodes) {
    for (const episode of record.episodes) {
      ...
      if (score < MIN_SHAPE_SCORE) continue
      ...
      hits.push({
        ...
        score,
        sharedConstraints: queryShape.constraints.filter((kind) =>
          episode.shape.constraints.includes(kind),
        ),
        sharedResolutions: queryShape.resolutions.filter((kind) =>
          episode.shape.resolutions.includes(kind),
        ),
        crossDomain:
          score >= CROSS_DOMAIN_SHAPE_THRESHOLD &&
...
/** 人话摘要。 */
function summarize(queryShape: EpisodeShape, hits: readonly AnalogyHit[]): string {
  if (hits.length === 0) {
