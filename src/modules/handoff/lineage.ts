...
/** 从摘要正文提取首行摘录（≤80 字符）。 */
function firstLineExcerpt(summary: string): string {
  const line = summary
    .split('\n')
    .map((l) => l.trim())
    .find((l) => l.length > 0)
...
export class LineageStore {
  ...
  constructor(domain: Domain) {
    this.table = domain.table<LineageEdge>('handoff-lineage')
  }

  /** 存储键：source>target:linkedAt（悬边 target 用 pending 占位）。 */
  private keyOf(source: string, target: string | null, linkedAt: number): string {
   ...
   */
  async recordEdge(source: string, target: string | null, summary: string): Promise<void> {
    ...
    if (entries.length <= EDGE_KEEP_LIMIT) return
    const stale = entries
   ...
   */
  async resolvePendingTargets(target: string): Promise<void> {
  ...
  async deletePendingEdges(): Promise<void> {
    const pending = this.table
      .entries()
      .filter(([, edge]) => edge.targetSessionId === null)
    for (const [key] of pending) await this.table.delete(key)
  }
   ...
   */
  ancestorsOf(sessionId: string, maxDepth = 10): LineageNode[] {
    ...
    for (let depth = 1; depth <= maxDepth && frontier.length > 0; depth += 1) {
      ...
      for (const node of frontier) {
        for (const edge of edges) {
          if (edge.targetSessionId !== node) continue
          if (visited.has(edge.sourceSessionId)) continue
   ...
   * 环安全（visited 去重）；按跳数升序、同跳数按时间升序。
   */
  descendantsOf(sessionId: string, maxDepth = 10): LineageNode[] {
    const edges = this.table.entries().map(([, edge]) => edge)
    const visited = new Set<string>([sessionId])
    let frontier = [sessionId]
    const result: LineageNode[] = []
    for (let depth = 1; depth <= maxDepth && frontier.length > 0; depth += 1) {
      ...
      for (const node of frontier) {
        for (const edge of edges) {
          if (edge.sourceSessionId !== node) continue
          if (edge.targetSessionId === null || visited.has(edge.targetSessionId)) continue
