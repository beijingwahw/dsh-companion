...
function startOfBeijingDay(ts: number): number {
...
export function parseTimeRange(text: string, now: number = Date.now()): TimeRange | undefined {
  ...
  // 命名窗口。
  const named: Record<string, () => TimeRange> = {
...
function weekWindow(now: number, offset: number): TimeRange {
  const today = startOfBeijingDay(now)
...
function monthWindow(now: number, offset: number): TimeRange {
  const p = beijingParts(now)
...
function chineseOrDigit(text: string): number {
  if (/^\d+$/.test(text)) return Number(text)
  const digits: Record<string, number> = {
    一: 1, 二: 2, 三: 3, 四: 4, 五: 5, 六: 6, 七: 7, 八: 8, 九: 9, 十: 10, 百: 100,
  }
  if (text.length === 1) return digits[text] ?? 0
  // 简单组合：十几 / 几十 / 几十几。
