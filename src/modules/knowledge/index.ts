...
import { buildEntityGraph, type EntityGraph } from '../../core/graph/graph.js'
import { detectCommunities, type CommunityReport } from '../../core/graph/louvain.js'
import {
  personalizedPageRank,
  seedFromQuery,
  type PageRankNode,
} from '../../core/graph/ppr.js'
import { detectTopicDrift, type DriftReport } from '../../core/cognition/drift.js'
import {
  consolidateMemories,
  type ConsolidationItem,
  type ConsolidationPlan,
} from '../../core/cognition/consolidation.js'
import {
  excavateKnowledge,
  type ArchaeologyReport,
  type ArchaeologySession,
} from '../../core/cognition/archaeology.js'
import {
  scanSessionEchoes,
  type RadarReport,
  type RadarSession,
} from '../../core/cognition/radar.js'
import { detectDarkMatter, type DarkMatterReport } from '../../core/cognition/darkmatter.js'
import {
  ENTITY_TYPES,
  entityTypeWeight,
  extractEntities,
...
export function apply(ctx: Context): void {
  ...
  function entityGraphOf(): EntityGraph {
    const { entities } = loadSnapshot()
  ...
  function continentsView(limit: number): CommunityReport & {
    communities: ReadonlyArray<{
      id: number
      size: number
      sessionCount: number
      internalWeight: number
      topEntities: ReadonlyArray<{ name: string; type: string }>
  ...
  function starMapView(queryText: string, limit: number): {
    query: string
  ...
  async function driftView(lambda: number): Promise<DriftReport> {
    const { entities } = loadSnapshot()
    // 会话 → 实体名列表（倒排投影：零转录重读，纯内存）。
    const sessionEntityNames = new Map<string, string[]>()
    for (const record of entities.values()) {
      for (const sessionId of Object.keys(record.sessions)) {
        const list = sessionEntityNames.get(sessionId) ?? []
        if (list.length === 0) sessionEntityNames.set(sessionId, list)
  ...
  function consolidationView(): ConsolidationPlan & {
    clusters: ReadonlyArray<{
      representativeId: string
      representativeProblem: string
  ...
  async function archaeologyView(windows: number): Promise<ArchaeologyReport> {
    const { entities } = loadSnapshot()
    // 会话 → 实体键列表（倒排投影：零转录重读，纯内存）。
    const sessionEntityKeys = new Map<string, string[]>()
    for (const [key, record] of entities) {
      for (const sessionId of Object.keys(record.sessions)) {
        const list = sessionEntityKeys.get(sessionId) ?? []
        if (list.length === 0) sessionEntityKeys.set(sessionId, list)
        list.push(key)
      }
    }
    const sessions = await ctx.sessionQuery.listSessions()
    const units: ArchaeologySession[] = sessions.map((session) => {
      const sessionId = String(session.id)
      return {
        id: sessionId,
        at: session.createdAt,
  ...
  > {
    ...
    const units: RadarSession[] = sessions.map((session) => {
      const sessionId = String(session.id)
  ...
  function darkMatterView(limit: number): DarkMatterReport {
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/knowledge/continents', async (_req, res, { query }) => {
        await ctx.companion.ready
        await syncKnowledge(false)
        const limit = parseLimitParam(
          query.get('limit') ?? undefined,
          DEFAULT_CONTINENTS_LIMIT,
          MAX_CONTINENTS_LIMIT,
        )
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/knowledge/starmap', async (_req, res, { query }) => {
        await ctx.companion.ready
        await syncKnowledge(false)
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/knowledge/drift', async (_req, res, { query }) => {
        await ctx.companion.ready
        await syncKnowledge(false)
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/knowledge/consolidation', async (_req, res) => {
        await ctx.companion.ready
        await syncKnowledge(false)
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/knowledge/archaeology', async (_req, res, { query }) => {
        await ctx.companion.ready
        await syncKnowledge(false)
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/knowledge/radar', async (_req, res, { query }) => {
        await ctx.companion.ready
        await syncKnowledge(false)
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/knowledge/darkmatter', async (_req, res, { query }) => {
        await ctx.companion.ready
        await syncKnowledge(false)
        const limit = parseLimitParam(
          query.get('limit') ?? undefined,
          DEFAULT_DARKMATTER_LIMIT,
          MAX_DARKMATTER_LIMIT,
        )
      ...
      ctx.commands.register({
        ...
        handler: async (): Promise<CommandResult> => {
          try {
            ...
            const lines: string[] = ['知识大陆（Louvain 社区发现）：', '']
            lines.push(
              `  图规模：${report.graph.nodes} 个实体、${report.graph.edges} 条共现边，模块度 Q = ${report.modularity.toFixed(4)}（${report.levels} 轮聚合）`,
            )
            ...
            for (const community of report.communities) {
              ...
              lines.push(
                `  ◆ 大陆 ${community.id + 1}：${community.size} 个实体，覆盖 ${community.sessionCount} 个会话`,
              )
              if (tops.length > 0) lines.push(`    核心实体：${tops}`)
            }
            lines.push('')
      ...
      ctx.commands.register({
        ...
        handler: async (invocation: CommandInvocation): Promise<CommandResult> => {
          ...
          try {
            ...
            const lines: string[] = [`星图导航（查询：${view.query}）：`, '']
            ...
            for (const node of view.nodes) {
              const hop =
                node.seed || node.hopDistance === null
                  ? ''
                  : node.hopDistance > 1
                    ? ` [${node.hopDistance} 跳]`
                    : ' *'
              lines.push(
                `  ${node.score >= view.nodes[0].score * 0.999 ? '★' : '·'} [${node.type}] ${node.name}${hop}（引力 ${(node.score * 100).toFixed(2)}%）`,
              )
            }
            lines.push('')
            lines.push(`  ${view.summary}`)
      ...
      ctx.commands.register({
        ...
        handler: async (invocation: CommandInvocation): Promise<CommandResult> => {
          ...
          try {
            ...
            const lines: string[] = ['主题漂移报告（BOCPD 变点检测）：', '']
            lines.push(
              `  样本：${report.unitCount} 个会话（词表 ${report.vocabSize} 项，λ = ${lambda}，期望运行长度 ${report.meanRunLength.toFixed(1)} 个会话）`,
            )
            if (report.changepoints.length === 0) {
              ...
              for (const cp of report.changepoints) {
                lines.push(
                  `  ⇄ ${formatBeijingTime(cp.at)}（置信度 ${(cp.probability * 100).toFixed(0)}%，切换强度 ${cp.jump.toFixed(2)} bit）`,
                )
                if (cp.beforeTerms.length > 0 || cp.afterTerms.length > 0) {
                  lines.push(
                    `    ${cp.beforeTerms.slice(0, 5).join('、') || '（无）'} → ${cp.afterTerms.slice(0, 5).join('、') || '（无）'}`,
                  )
                }
              }
            }
            lines.push('')
            lines.push(
              `当前主题段：已持续 ${report.currentRunLength} 个会话，特征词 ${report.currentRunTopTerms.slice(0, 6).join('、') || '（无）'}`,
            )
      ...
      ctx.commands.register({
        ...
        handler: async (): Promise<CommandResult> => {
          try {
            ...
            const lines: string[] = ['记忆固化报告（MinHash-LSH 近重复聚类）：', '']
            lines.push(
              `  片段库：${plan.items} 条，近重复簇 ${plan.clusters.length} 组，折叠冗余 ${plan.duplicates} 条（压缩率 ${(plan.compression * 100).toFixed(0)}%），唯一 ${plan.unique} 条`,
            )
            if (plan.clusters.length > 0) {
              ...
              for (const cluster of plan.clusters) {
                const problem =
                  cluster.representativeProblem.length > 60
                    ? `${cluster.representativeProblem.slice(0, 60)}…`
                    : cluster.representativeProblem
                lines.push(
                  `  ◆ [×${cluster.reinforcement}] ${problem}（紧致度 ${(cluster.cohesion * 100).toFixed(0)}%，${formatBeijingTime(cluster.firstSeenAt)} 首现）`,
                )
              }
            }
            lines.push('')
      ...
      ctx.commands.register({
        ...
        handler: async (): Promise<CommandResult> => {
          try {
            ...
            const lines: string[] = ['知识考古报告（板块构造学）：', '']
            for (const w of report.windows) {
              const continents = w.continents
                ...
                .join('、')
              lines.push(
                `  纪元 ${w.index + 1}（${formatBeijingTime(w.fromAt)} 起，${w.sessions} 会话）：` +
      ...
      ctx.commands.register({
        ...
        handler: async (): Promise<CommandResult> => {
          try {
            ...
            const lines: string[] = ['回声雷达报告（会话级复发检测）：', '']
            lines.push(
              `  会话库：${report.sessions} 场，回声簇 ${report.clusters.length} 组，冗余 ${report.duplicates} 场` +
            ...
            for (const cluster of report.clusters) {
              ...
              const period =
                cluster.recurrenceDays !== null ? `，复发周期约 ${cluster.recurrenceDays} 天` : ''
              const action =
                cluster.action === 'template' ? ' → 建议固化为交接模板' : ' → 保持关注'
              lines.push(
                `  ◆ [×${cluster.occurrences}] ${title}（紧致度 ${(cluster.cohesion * 100).toFixed(0)}%${period}）${action}`,
              )
            }
            lines.push('')
            lines.push(`  ${report.summary}`)
      ...
      ctx.commands.register({
        ...
        handler: async (): Promise<CommandResult> => {
          try {
            ...
            const lines: string[] = ['知识暗物质报告（缺失连接预测）：', '']
            lines.push(
              `  图规模：${report.graph.nodes} 实体、${report.graph.edges} 条共现边；` +
            ...
            for (const link of report.links) {
              lines.push(
                `  ◆ ${link.u} ↔ ${link.v}（共同邻居 ${link.commonNeighbors}，` +
...
function parseLambdaParam(value: string | undefined): number {
  if (value === undefined || value === '') return DEFAULT_DRIFT_LAMBDA
  const parsed = Number(value)
  if (!Number.isInteger(parsed) || parsed < MIN_DRIFT_LAMBDA || parsed > MAX_DRIFT_LAMBDA) {
...
function parseWindowsParam(value: string | undefined): number {
  if (value === undefined || value === '') return DEFAULT_ARCHAEOLOGY_WINDOWS
  const parsed = Number(value)
  if (
    ...
    throw new HttpError(
      `windows 必须是 [${MIN_ARCHAEOLOGY_WINDOWS}, ${MAX_ARCHAEOLOGY_WINDOWS}] 内的整数`,
      400,
    )
  }
  return parsed
...
function parseRadarDaysParam(value: string | undefined): number {
  if (value === undefined || value === '') return DEFAULT_RADAR_DAYS
  const parsed = Number(value)
