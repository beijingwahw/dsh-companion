...
export function chunkTranscript(
  ...
  targetChars: number,
): readonly (readonly TranscriptTurn[])[] {
  const chunks: (readonly TranscriptTurn[])[] = []
...
export function scoreText(question: string, text: string): number {
  ...
  let questionNorm = 0
  for (const count of questionGrams.values()) questionNorm += count * count
  let textNorm = 0
  for (const count of textGrams.values()) textNorm += count * count
  const semantic =
    questionNorm > 0 && textNorm > 0 ? dot / Math.sqrt(questionNorm * textNorm) : 0
...
export function selectEvidence(
  candidates: readonly EvidenceChunk[],
  ...
  for (const chunk of sorted) {
    if (picked.length >= options.maxChunks) break
    ...
    if (totalChars >= options.charBudget) break
  }
  return picked
}
