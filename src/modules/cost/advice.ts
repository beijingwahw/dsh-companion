...
export interface CostAdviceSnapshot {
  ...
  /** 同厂商更低价的候选模型（无则缺省）。 */
  readonly cheaperModel?: { readonly model: string; readonly priceRatio: number }
}
...
const SEVERITY_ORDER: Record<CostAdviceCard['severity'], number> = {
  critical: 0,
  recommended: 1,
  info: 2,
}
 ...
 */
export function composeCostAdvice(snapshot: CostAdviceSnapshot): CostAdviceReport {
  const cards: CostAdviceCard[] = []
  ...
  if (snapshot.monthlyBudget > 0 && snapshot.monthProjection !== undefined) {
    ...
    if (projection > snapshot.monthlyBudget) {
      ...
      cards.push({
        ...
        action:
          `月末投影 ${projection.toFixed(2)} 元将超出月预算 ${snapshot.monthlyBudget} 元` +
  ...
  if (snapshot.peakNow && snapshot.deferrableRatio > 0.05 && snapshot.peakPremium > 0) {
    ...
    if (saving >= 0.5) {
      cards.push({
        ...
        action:
          `当前处于高峰时段，近 30 天 ${Math.round(snapshot.deferrableRatio * 100)}% 调用可延迟；` +
  ...
  if (snapshot.cacheHitRate < 0.9 && snapshot.cacheMissTokens > 0 && snapshot.cacheDiscount > 0) {
    ...
    if (saving >= 0.5) {
      cards.push({
        ...
        estimatedSavingCny: Number(saving.toFixed(2)),
        action:
          `当前缓存命中率 ${Math.round(snapshot.cacheHitRate * 100)}%；命中率的提升路径是` +
  ...
  if (snapshot.cheaperModel !== undefined && snapshot.cheaperModel.priceRatio < 1) {
    ...
    if (saving >= 1) {
      cards.push({
        ...
        estimatedSavingCny: Number(saving.toFixed(2)),
        action:
          `主力模型 ${snapshot.primaryModel} 的同厂商候选 ${snapshot.cheaperModel.model} ` +
...
function finish(cards: CostAdviceCard[]): CostAdviceReport {
