...
export function apply(ctx: Context): void {
    ...
    () =>
      ctx.companion.http.add('POST', '/cost/settings', async (_req, res, { body }) => {
        ...
        const pricingOverrides =
          record.pricing !== undefined ? parsePricing(record.pricing) : undefined
  ...
  async function recentWindow(days: number): Promise<DailyUsage[]> {
    const { usage } = await ctx.companion.ready
    const today = beijingDayKey(Date.now())
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/cost/forecast', async (_req, res, { query }) => {
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/cost/anomalies', async (_req, res, { query }) => {
        const days = parseIntParam(query.get('days') ?? undefined, 30, 'days', 1, 365)
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('POST', '/cost/what-if', async (_req, res, { body }) => {
        const record = readObject(body)
        ...
        if (record.callVolumeFactor !== undefined) {
          if (
            typeof record.callVolumeFactor !== 'number' ||
        ...
        sendJson(
          res,
          200,
          whatIfSimulate(rows, scenario, (model, atMs) => ctx.companion.prices.resolve(model, atMs), days),
        )
      }),
  ...
  ctx.effect(
    () =>
      ctx.companion.http.add('GET', '/cost/attribution', async (_req, res, { query }) => {
        ...
        sendJson(res, 200, attributeChange(
          usage.range(currentFrom, today),
          usage.range(previousFrom, previousTo),
        ))
      }),
 ...
 */
function parseModelShift(value: unknown): WhatIfScenario['modelShift'] {
  if (!Array.isArray(value)) {
  ...
  const shifts: Array<{ from: string; to: string; ratio: number }> = []
  for (let index = 0; index < list.length; index += 1) {
    const raw: unknown = list[index]
    if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
