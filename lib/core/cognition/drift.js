 ...
 */
export function detectTopicDrift(units, options = {}) {
    ...
    const ordered = [...units].sort((a, b) => a.at - b.at || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
    const window = ordered.slice(-maxUnits);
    const count = window.length;
    const now = count > 0 ? window[count - 1].at : 0;
    if (count < 2)
        return trivialReport(now, count);
    ...
    const unitIds = unitTerms.map((terms) => {
        const ids = [];
        for (const term of terms) {
    ...
    for (let t = 1; t < count; t += 1) {
        ...
        for (let k = t; k >= 1; k -= 1)
            logR[k] = weights[k] - logZ;
        logR[0] = logCp - logZ;
        const alphaBase = t * stride;
        for (let k = 0; k <= t; k += 1)
            logAlphaPost[alphaBase + k] = logR[k];
    ...
    const changepoints = adopted.map((index) => {
        const segIndex = bounds.indexOf(index) - 1;
        const before = segmentTf[segIndex];
        const after = segmentTf[segIndex + 1];
        const jump = before !== undefined && after !== undefined && before.total > 0 && after.total > 0
    ...
    const lastSurprise = tokenCounts[count - 1] > 0
...
function topTermsOf(tf, total, globalTf, globalTot, vocabTerms, topK) {
    if (total === 0 || globalTot === 0)
        return [];
    const scored = [];
    for (const [w, c] of tf) {
...
function jsDivergence(p, pTot, q, qTot) {
    let js = 0;
    for (const [w, pc] of p) {
        const pw = pc / pTot;
...
/** 数据不足时的平凡报告。 */
function trivialReport(now, count) {
    return {
        now,
        unitCount: count,
        vocabSize: 0,
...
/** 漂移报告人话摘要。 */
function summarizeDrift(count, cpCount, lastSeg, lastSurprise) {
    if (cpCount === 0) {
    ...
    const surprisePart = lastSurprise > 8 ? `；最近一条内容偏离当前主题较远（${lastSurprise.toFixed(1)} bit/词元）` : '';
