 ...
 */
export function minhashSignature(terms, size) {
 ...
 */
export function consolidateMemories(items, options = {}) {
    ...
    if (n < 2)
        return emptyPlan(n);
    // token 集合 + 签名。
    const tokenSets = pool.map((item) => new Set(tokenize(item.text)));
    const signatures = tokenSets.map((set) => minhashSignature(set, size));
    // ---- LSH 分带：任一带相等 → 候选对 ----
    const candidates = new Set(); // 编码 i·n + j（i < j）
    ...
    clusters.sort((a, b) => b.reinforcement - a.reinforcement || (a.representativeId < b.representativeId ? -1 : 1));
    return {
        items: n,
        clusters,
        duplicates,
...
/** 固化计划人话摘要。 */
function summarizeConsolidation(n, clusters, duplicates) {
