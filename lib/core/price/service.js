...
export function costOf(price, usage) {
    if (price === undefined)
        return 0;
    return (usage.inputTokens * price.inputMiss +
...
export function isPeakTimeAt(atMs, windows = DEFAULT_PEAK_WINDOWS) {
    const shifted = new Date(atMs + 8 * 3600_000);
    const hour = shifted.getUTCHours() + shifted.getUTCMinutes() / 60;
    return windows.some(([start, end]) => start <= end ? hour >= start && hour < end : hour >= start || hour < end);
}
/**
 ...
 * 公布分时价时其余模型被错误计为 0 元。
 */
export function resolvePrice(sheet, model, atMs) {
    if (sheet.scheduled !== undefined && beijingDayKey(atMs) >= sheet.scheduled.effective) {
        const table = isPeakTimeAt(atMs, sheet.scheduled.peakWindows ?? DEFAULT_PEAK_WINDOWS)
            ? sheet.scheduled.peak
            : sheet.scheduled.offPeak;
        return table[model] ?? sheet.current[model];
    }
    return sheet.current[model];
}
/** 带超时抓取 URL 文本。 */
export async function fetchText(url, timeoutMs, headers = {}) {
    ...
    if (!response.ok)
        throw new Error(`pricing fetch failed: HTTP ${response.status}`);
    return response.text();
}
const isModelId = (text) => /^deepseek-[\w.-]+$/i.test(text.trim());
/** 解析 "0.05元" / "¥1.5" 风格单元格为数字（DeepSeek 页）。 */
function parseDeepSeekPrice(text) {
 ...
 * （调用方保留上一份有效表）。
 */
export function parsePriceSheet(html, url) {
    ...
    for (const grid of grids) {
        ...
        header.forEach((text, col) => {
            if (isModelId(text))
                modelCols.push([col, text.trim().toLowerCase()]);
        });
        ...
        for (const row of grid.slice(1)) {
            ...
            const pick = (col) => parseDeepSeekPrice(row[col] ?? '');
            for (const [col, model] of modelCols) {
                ...
                if (label.includes('缓存未命中')) {
                    ...
                    if (value !== undefined)
                        existing.inputMiss = value;
                }
                else if (label.includes('缓存命中')) {
                    const value = pick(col);
                    if (value !== undefined)
                        existing.inputCacheHit = value;
                }
    ...
    for (const grid of grids) {
        ...
        if (header === undefined)
            continue;
        if (!header.some((text) => text.includes('缓存命中')) || !header.some((text) => text.includes('输出')))
            continue;
        const offPeak = {};
        const peak = {};
        for (const row of grid.slice(1)) {
            const modelCell = row[0] ?? '';
            if (!isModelId(modelCell) || row.length < 5)
                continue;
    ...
    if (scheduled !== undefined) {
        ...
        if (windows !== null) {
            const nums = windows.slice(1, 9).map(Number);
...
function canonicalSheet(sheet) {
    const norm = (table) => Object.fromEntries(Object.entries(table)
    ...
    return JSON.stringify({
        ...
        scheduled: sheet.scheduled === undefined
            ...
            : {
                ...
                // 规范化为「缺省窗口也写出显式值」，避免持久化快照（无窗口）
                // 与实时表（带窗口）内容相同却被误判为变更、每轮触发落盘。
                windows: sheet.scheduled.peakWindows ?? DEFAULT_PEAK_WINDOWS,
...
function summarizeSheet(sheet) {
    ...
    if (sheet.scheduled !== undefined)
        parts.push(`峰谷自 ${sheet.scheduled.effective} 生效`);
    return parts.join(', ');
}
/** 对某表键做最长前缀匹配。 */
function matchByPrefix(table, model) {
...
function sanitizePriceTable(raw) {
    ...
    for (const [model, value] of Object.entries(raw)) {
        if (typeof value !== 'object' || value === null || Array.isArray(value))
            continue;
        const entry = value;
        // 严格类型检查：禁止 Number() 强转把 null→0、["0.5"]→0.5 当合法价格。
        if (typeof entry.inputCacheHit !== 'number' || typeof entry.inputMiss !== 'number' || typeof entry.output !== 'number')
            continue;
        const { inputCacheHit: hit, inputMiss: miss, output } = entry;
        if (!Number.isFinite(hit) || !Number.isFinite(miss) || !Number.isFinite(output))
            continue;
...
/** 校验高峰窗口二元组：0 ≤ 起止 ≤ 24 且起 ≠ 止（支持跨午夜 start > end）。 */
function sanitizePeakWindow(w) {
...
export function sanitizePriceSheet(raw) {
    if (typeof raw !== 'object' || raw === null || Array.isArray(raw))
        return undefined;
    const entry = raw;
    ...
    if (Object.keys(current).length === 0)
        return undefined;
    const sheet = { source: 'live', current };
    if (typeof entry.fetchedAt === 'number' && Number.isFinite(entry.fetchedAt))
        sheet.fetchedAt = entry.fetchedAt;
    if (typeof entry.sourceUrl === 'string')
        sheet.sourceUrl = entry.sourceUrl;
    if (typeof entry.scheduled === 'object' && entry.scheduled !== null && !Array.isArray(entry.scheduled)) {
        const sched = entry.scheduled;
        const offPeak = sanitizePriceTable(sched.offPeak);
        const peak = sanitizePriceTable(sched.peak);
        // peak 与 offPeak 双双非空才接受分时计划（与 parsePriceSheet 同口径），
        // 防止残缺快照把空闲时段整体清零。
        const effectiveOk = typeof sched.effective === 'string' && isRealDayKey(sched.effective);
        if (effectiveOk && Object.keys(peak).length > 0 && Object.keys(offPeak).length > 0) {
            const scheduled = { effective: sched.effective, offPeak, peak };
            if (Array.isArray(sched.peakWindows)) {
                const windows = [];
                for (const w of sched.peakWindows) {
                    const window = sanitizePeakWindow(w);
                    if (window !== undefined)
                        windows.push(window);
                }
                if (windows.length > 0)
                    scheduled.peakWindows = windows;
            }
...
export class PriceService {
    ...
    /** 官方价格内容变化回调（供持久化与提示）。 */
    onChanged;
    constructor(pricingUrl, timeoutMs, log) {
    ...
    vendorPricing(atMs) {
        ...
        const deepseekModels = { ...resolveTable(this.sheet, atMs) };
        for (const key of Object.keys(this.overrides)) {
            if (vendorOf(key) === 'deepseek')
                deepseekModels[key] = this.overrides[key];
        }
        result.push({
            id: 'deepseek',
        ...
        for (const [vendorId, info] of Object.entries(VENDORS)) {
            ...
            let source = live !== undefined ? 'live' : 'builtin';
    ...
    async fetchVendorTable(vendorId) {
        const info = VENDORS[vendorId];
        const kind = info.fetchKind ?? 'html';
        if (kind === 'ernie-cdn') {
            const json = JSON.parse(await fetchText(info.dataSource ?? info.pricingUrl, this.timeoutMs, { 'user-agent': BROWSER_UA }));
        ...
        if (kind === 'kimi-rsc') {
            ...
            const pages = [...new Set([...index.matchAll(/href:`(\/pricing\/[\w-]+)`/g)].map((m) => m[1] ?? ''))].filter((p) => p !== '/pricing/chat');
            const table = {};
...
/**
 * 某时刻下生效的平价表（分时计划感知）。
 * 以平价表为基底、分时表覆盖同键条目合并而成，保证未参与分时的
 * 模型仍按平价计费（面板展示与计费同口径）。
 */
function resolveTable(sheet, atMs) {
    if (sheet.scheduled !== undefined && beijingDayKey(atMs) >= sheet.scheduled.effective) {
        const table = isPeakTimeAt(atMs, sheet.scheduled.peakWindows ?? DEFAULT_PEAK_WINDOWS)
            ? sheet.scheduled.peak
            : sheet.scheduled.offPeak;
        return { ...sheet.current, ...table };
    }
