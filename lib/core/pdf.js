 ...
 */
export function isLatin1Safe(text) {
...
export function buildSimplePdf(title, lines) {
    ...
    for (const pageLines of pages) {
        ...
        // /Length 使用字节长度（escapePdfText 已将全部非 ASCII 转为八进制转义，
        // 内容流为纯 ASCII，字符数==字节数；encode 复核以防御未来改动）。
    ...
    const parts = [header, catalog, pagesObj, font, ...objects];
    const offsets = [0]; // offsets[id] = 对象 `id 0 obj` 的字节偏移（0 号位占位）
...
function buildContentStream(lines) {
    ...
    const parts = [`BT /F1 ${FONT_SIZE} Tf ${LEADING} TL ${MARGIN} ${startY} Td`];
    for (const line of lines) {
...
function escapePdfText(text) {
    ...
    for (let i = 0; i < text.length; i += 1) {
        ...
        // 0xA0–0xFF（WinAnsi 与 Latin-1 码位一致）：必须以八进制转义按单字节
        // 值写入——PDF 字符串字节按 WinAnsi 解码，若直接输出原始字符，整份
        // 文件经 UTF-8 编码后这些字符变成双字节序列（0xC3 0xA9…），查看器
        // 逐字节按 WinAnsi 解读即为乱码（é → Ã©）。转义后内容流为纯 ASCII，
        // 与文件编码彻底解耦。
        if (code >= 0xa0) {
            out += `\\${code.toString(8).padStart(3, '0')}`;
            continue;
 ...
 */
export function buildPrintHtml(title, bodyHtml) {
    return `<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8" />
