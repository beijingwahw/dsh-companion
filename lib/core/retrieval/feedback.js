...
export function sanitizeFeedbackProfile(raw) {
    if (typeof raw !== 'object' || raw === null)
        return undefined;
    const record = raw;
    if (typeof record.lastAt !== 'number' || !Number.isFinite(record.lastAt))
        return undefined;
    const terms = {};
    if (typeof record.terms === 'object' && record.terms !== null) {
        for (const [term, count] of Object.entries(record.terms)) {
 ...
 */
export function recordClick(profile, queryText, at) {
    ...
    if (entries.length > TERM_CAP) {
        ...
        const kept = {};
        for (const [term, count] of entries.slice(0, TERM_CAP))
            kept[term] = count;
 ...
 */
export function feedbackBoost(profile, queryText, now) {
    if (profile === undefined || profile.clicks <= 0)
        return 0;
