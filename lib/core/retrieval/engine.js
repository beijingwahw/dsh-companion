 ...
 */
export function recencyBoostFor(doc, recency) {
...
export function sanitizeIndexedDoc(raw) {
    ...
    if (typeof record.termFreqs === 'object' && record.termFreqs !== null) {
        for (const [term, freq] of Object.entries(record.termFreqs)) {
    ...
    if (typeof record.grams === 'object' && record.grams !== null) {
        for (const [gram, freq] of Object.entries(record.grams)) {
            if (typeof freq === 'number' && Number.isFinite(freq) && freq > 0)
                grams[gram] = freq;
        }
    }
...
export function buildIndexedDoc(meta, text) {
    ...
    *docsWithTerm(term) {
    ...
    removeStats(doc) {
        ...
        for (const gram of Object.keys(doc.grams)) {
            ...
            if (df <= 1)
                this.gramDf.delete(gram);
            else
                this.gramDf.set(gram, df - 1);
        }
    }
     ...
     */
    search(queryText, limit, filter, recency) {
        const trimmed = queryText.trim();
        if (trimmed.length === 0 || this.docs.size === 0)
            return [];
        ...
        if (filter !== undefined) {
            ...
            for (const doc of this.docs.values()) {
                if (filter(doc))
                    visible.set(doc.sessionId, doc);
            }
            if (visible.size === 0)
                return [];
        }
        const stats = visible !== undefined ? this.viewStatsOf(visible) : this.globalStats();
        const corpus = visible !== undefined ? visible.values() : this.docs.values();
     ...
     */
    lexicalRankingOf(queryText, corpus, stats) {
        ...
        for (const doc of corpus) {
            ...
            if (score > 0)
                scores.set(doc.sessionId, score);
        }
        return scores;
    }
    /**
     ...
     * 缓解固定维度下的偏置聚集。
     */
    semanticRankingOf(queryText, corpus, stats) {
        const docCount = stats.docCount;
        const gramIdf = (gram) => Math.log(1 + docCount / (1 + (stats.gramDf.get(gram) ?? 0)));
        const hashGram = (gram) => {
        ...
        for (const doc of corpus) {
            ...
            let docNorm = 0;
            for (const value of docVec.values())
                docNorm += value * value;
            ...
            if (cosine > 0)
                scores.set(doc.sessionId, cosine);
        }
        return scores;
    }
}
 ...
 */
export function docGramVector(doc) {
 ...
 */
export function textGramVector(text) {
    const vector = new Map();
    for (const [gram, freq] of charTrigrams(text)) {
        const hash = fnv1a32(gram);
        const dim = hash & (VECTOR_DIM - 1);
 ...
 */
export function buildSnippet(text, queryText) {
    ...
    for (let start = 0; start < trimmed.length; start += step) {
        ...
        if (start + windowSize >= trimmed.length)
            break;
    }
