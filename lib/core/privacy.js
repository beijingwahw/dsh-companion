...
function normalizeDigitSeparators(text) {
    return text.replace(/\d(?:[\s-]*\d)+/g, (run) => {
        ...
        if (!MERGEABLE_WIDTHS.has(digitCount))
            return run;
...
export function redactText(text) {
    const stats = { phone: 0, email: 0, idCard: 0, bankCard: 0, ipv4: 0, secret: 0 };
    ...
    let result = normalized.replace(API_KEY_RE, (_m, prefix, _rest) => {
        stats.secret += 1;
        return `${prefix}****`;
    });
    ...
    result = result.replace(BEARER_RE, (_m, scheme, _token) => {
        stats.secret += 1;
        return `${scheme}****`;
    });
    ...
    // 被二次打码造成重复计数。
    result = result.replace(ID_CARD_RE, (_m, head, _month, _day, tail) => {
        stats.idCard += 1;
        return `${head}********${tail}`;
    });
    result = result.replace(OLD_ID_CARD_RE, (_m, head, _month, _day, tail) => {
        stats.idCard += 1;
        return `${head}********${tail}`;
    });
    ...
    result = result.replace(IPV4_RE, (match, head, c, d) => {
    ...
    result = result.replace(PHONE_RE, (_m, prefix, head, tail) => {
        stats.phone += 1;
...
export function hasRedactions(stats) {
    return (stats.phone + stats.email + stats.idCard + stats.bankCard + stats.ipv4 + stats.secret > 0);
}
