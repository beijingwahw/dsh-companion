...
import { buildSynthesisPrompt, chunkTranscript, scoreText, } from './retrieval.js';
import { analyzeEvolution } from '../../core/synthesis/evolution.js';
...
export function apply(ctx) {
     ...
     * 两条通道各自容错：任一失败不阻塞另一条。
     */
    async function recallSessions(question) {
        ...
        try {
            ...
            for (const session of sorted.slice(0, RECENT_FALLBACK)) {
                if (!merged.has(String(session.id)))
    ...
    function budgetTurns(turns) {
        ...
        let headChars = 0;
        for (const turn of turns) {
            if (headChars + turn.text.length > halfBudget)
                break;
            head.push(turn);
        ...
        for (let index = turns.length - 1; index >= 0; index -= 1) {
            ...
            if (head.includes(turn))
                break;
            if (tailChars + turn.text.length > halfBudget)
                break;
            tail.unshift(turn);
     ...
     */
    async function collectEvidence(question) {
        // 1) 候选召回。
        ...
        const evidence = selection.selected.map((item) => {
    ...
    async function answerQuestion(question) {
        ...
        const evolution = analyzeEvolution(evidence.map((chunk) => ({
        ...
        for (const chunk of evidence) {
            if (seenSessions.has(chunk.sessionId))
                continue;
     ...
     */
    async function previewQuestion(question) {
        const { candidates, chunks, selection, evidence } = await collectEvidence(question);
    ...
    // HTTP 端点（注册即 effect）
    // ------------------------------------------------------------------
    ctx.effect(() => ctx.companion.http.add('POST', '/synthesis/answer', async (_req, res, { body }) => {
    ...
    ctx.effect(() => ctx.companion.http.add('POST', '/synthesis/preview', async (_req, res, { body }) => {
        const record = readObject(body);
        const question = requireString(record.question, 'question');
        if (question.length > QUESTION_MAX_CHARS) {
            throw new HttpError(`question 长度不能超过 ${QUESTION_MAX_CHARS} 字符`, 400);
    ...
    ctx.effect(() => ctx.commands.register({
        ...
        handler: async (invocation) => {
            ...
            try {
                ...
                lines.push(`（检索 ${result.stats.candidates} 个会话 · ${result.stats.evidenceChunks} 个证据片段 · ${result.model}）`);
                lines.push('');
                lines.push('证据来源：');
