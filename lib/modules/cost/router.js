...
/** 量化单元抬升所在层的最大星高记录。 */
function applyQuantifier(unitHeight, stack) {
...
function assertSafeStarHeight(pattern) {
    if (starHeight(pattern) > MAX_STAR_HEIGHT) {
        throw new Error(`pattern 存在嵌套无界量词（星高 >${MAX_STAR_HEIGHT}），有灾难性回溯（ReDoS）风险：${pattern}`);
    }
}
...
export class ModelRouter {
    ...
    matchPattern(hint, pattern) {
        ...
        if (regex === undefined)
            return false;
        const subject = hint.length > MAX_REGEX_TEST_INPUT ? hint.slice(0, MAX_REGEX_TEST_INPUT) : hint;
