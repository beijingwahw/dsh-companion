...
export class BudgetGuard {
    ...
    applySettlement(actualCny) {
        ...
        if (this.dailyCache?.periodKey === dayKey)
            this.dailyCache = undefined;
        if (this.monthlyCache?.periodKey === monthKey)
            this.monthlyCache = undefined;
    }
    ...
    async alertOnce(tier, period, level, spentCny, budgetCny, includesReserved = false) {
        ...
        try {
            await this.table.update(storeKey, (prev) => {
                ...
                const base = carry ?? { alerted80: false, alerted100: false, budgetCny };
                return level === 80
