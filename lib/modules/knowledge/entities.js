...
export function extractEntities(text) {
    ...
    const found = new Map();
    /** 尝试记录一个实体命中（区间未被消费时）。 */
    const record = (start, end, rawName, type) => {
        if (rawName.length < 2 || rawName.length > MAX_NAME_LENGTH)
            return;
    ...
    for (const match of sanitized.matchAll(/\b[A-Z][A-Z0-9]{1,7}\b/g)) {
        if (ACRONYM_STOPWORDS.has(match[0]))
            continue;
    ...
    for (const match of sanitized.matchAll(/\b[A-Z][a-z]{2,15}\b/g)) {
        if (CAPITALIZED_STOPWORDS.has(match[0]))
            continue;
    ...
    for (const [key, entity] of [...merged.entries()]) {
        ...
        if (canonicalKey === undefined || canonicalKey === key)
            continue;
...
function findCanonicalKey(merged, lowerName) {
    let bestKey;
    let bestPriority = Number.POSITIVE_INFINITY;
    for (const [key, entity] of merged) {
        if (entity.lower !== lowerName)
            continue;
...
/** 实体的类型权重（打分与展示徽章共用）。 */
export function entityTypeWeight(type) {
