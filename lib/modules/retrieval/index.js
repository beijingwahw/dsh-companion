import { buildIndexedDoc, buildSnippet, docGramVector, HybridRetrievalIndex, recencyBoostFor, sanitizeIndexedDoc, sparseCosine, textGramVector, } from '../../core/retrieval/engine.js';
import { expandQuery } from '../../core/retrieval/query.js';
import { diagnoseRetrieval, VERDICT_LABELS, } from '../../core/retrieval/quality.js';
import { feedbackBoost, recordClick, sanitizeFeedbackProfile, } from '../../core/retrieval/feedback.js';
import { DEFAULT_MMR_LAMBDA, diversityPoolSize, mmrSelect, MMR_MIN_POOL, } from '../../core/retrieval/diversity.js';
import { clusterSessions } from '../../core/retrieval/clusters.js';
import { suggestQueries } from '../../core/retrieval/suggest.js';
import { explainHit } from '../../core/retrieval/explain.js';
import { relaxQuery } from '../../core/retrieval/rescue.js';
import { negationFilterOf, parseNegations } from '../../core/retrieval/negate.js';
import { analyzeBlindSpots, sanitizeMissRecord, } from '../../core/retrieval/blindspots.js';
import { buildRankerFeatures, emptyRankerModel, learnedMultiplier, rankerDiagnostics, RANKER_NEGATIVE_WEIGHT, sanitizeRankerModel, updateRanker, } from '../../core/retrieval/ranker.js';
import { blindSpotInsights, composePulse, indexInsights, learningInsights, } from '../../core/insights/pulse.js';
...
export function apply(ctx) {
    ...
    let rankerModel = emptyRankerModel();
    /**
    ...
    function openTable(domain) {
        return domain.table('retrieval-index');
    }
    void ctx.companion.ready
        ...
        feedbackTable = domain.table('retrieval-feedback');
    ...
    function budgetTranscript(text) {
        if (text.length <= INDEX_TRANSCRIPT_BUDGET)
            return text;
        ...
        return (text.slice(0, headLength) + INDEX_TRUNCATION_NOTICE + text.slice(text.length - tailLength));
    }
    /**
     * 索引对账（增量同步）：
     * - 已索引但已不存在的会话 → 删除索引；
    ...
    async function syncIndex(force) {
        ...
        syncInFlight = (async () => {
            ...
            for (const [sessionId, session] of live) {
                ...
                const text = budgetTranscript(formatTranscript(transcriptFromLog(snapshot), { timestamps: false }));
    ...
    async function hybridSearch(params) {
        ...
        const dateFilter = (doc) => {
            if (params.from !== undefined && doc.createdAt < params.from)
                return false;
            if (params.to !== undefined && doc.createdAt > params.to)
                return false;
            return true;
        };
        const docFilter = negationKeep !== undefined
            ? (doc) => dateFilter(doc) && negationKeep(doc.termFreqs)
            : dateFilter;
        /** 扩展 + 检索的单轮执行（首轮与救援轮共用）。 */
        const runSearch = (queryText) => {
            // 轴线 8：查询智能——拼写纠错 + 语料共现扩展（倒排表直达，纯本地）。
            const expanded = expandQuery(queryText, corpus);
            const hits = index.search(expanded.queryText, limit, docFilter, 
        ...
        if (pool.length >= MMR_MIN_POOL) {
            ...
            for (const hit of pool) {
                const doc = index.get(hit.sessionId);
                if (doc !== undefined)
                    vectors.set(hit.sessionId, docGramVector(doc));
            }
            ordered = mmrSelect(pool, (hit) => hit.score, (a, b) => {
        ...
        await Promise.all(detailed.map(async (hit) => {
            try {
        ...
        return {
            hits: result,
            expansion: expanded.notes.length > 0
     ...
     */
    async function blindSpotAnalysis() {
        await ctx.companion.ready;
     ...
     * @returns 更新后的累计点击次数。
     */
    async function recordFeedback(sessionId, queryText) {
        await ctx.companion.ready;
        ...
        if (context && context.query === queryText.trim()) {
            ...
            if (position >= 0) {
                ...
                for (let i = 0; i < position; i += 1) {
                    model = updateRanker(model, context.entries[i].features, 0, RANKER_NEGATIVE_WEIGHT);
                }
                rankerModel = model;
     ...
     */
    async function knowledgeMap(minSize) {
        await ctx.companion.ready;
        await syncIndex(false);
    ...
    // HTTP 端点（注册即 effect）
    // ------------------------------------------------------------------
    ctx.effect(() => ctx.companion.http.add('GET', '/retrieval/search', async (_req, res, { query }) => {
    ...
    ctx.effect(() => ctx.companion.http.add('GET', '/retrieval/suggest', async (_req, res, { query }) => {
    ...
    ctx.effect(() => ctx.companion.http.add('GET', '/retrieval/blindspots', async (_req, res) => {
        sendJson(res, 200, await blindSpotAnalysis());
    }), 'companion.retrieval-http-blindspots');
    ctx.effect(() => ctx.companion.http.add('GET', '/retrieval/insights', async (_req, res) => {
        sendJson(res, 200, await pulse());
    }), 'companion.retrieval-http-insights');
    ctx.effect(() => ctx.companion.http.add('GET', '/retrieval/status', async (_req, res) => {
        await ctx.companion.ready;
        sendJson(res, 200, {
            indexed: index.size,
            lastSyncAt: lastSyncAt > 0 ? lastSyncAt : null,
    ...
    ctx.effect(() => ctx.companion.http.add('POST', '/retrieval/reindex', async (_req, res) => {
        await ctx.companion.ready;
        // 强制对账：清内存统计后全量重建（持久化表逐条覆盖）。
        const result = await syncIndex(true);
        sendJson(res, 200, { ok: true, ...result });
    }), 'companion.retrieval-http-reindex');
    ctx.effect(() => ctx.companion.http.add('POST', '/retrieval/feedback', async (_req, res, { body }) => {
        await ctx.companion.ready;
    ...
    ctx.effect(() => ctx.companion.http.add('GET', '/retrieval/clusters', async (_req, res, { query }) => {
    ...
    ctx.effect(() => ctx.commands.register({
        ...
        handler: async (invocation) => {
            ...
            try {
                const { hits, expansion, diagnostics, feedback, diversity, rescue } = await hybridSearch({ query: queryText });
                if (hits.length === 0) {
                ...
                const lines = [`语义检索到 ${hits.length} 个对话（混合排序）：`];
                if (rescue !== undefined) {
                ...
                lines.push(`质量: 相关度 ${strength}%（${VERDICT_LABELS[diagnostics.verdict]}）· 词法覆盖 ${Math.round(diagnostics.lexicalCoverage * 100)}% · 语义覆盖 ${Math.round(diagnostics.semanticCoverage * 100)}%`);
                let rank = 1;
                for (const hit of hits) {
                    lines.push(`${rank}. ${hit.session.title ?? '未命名对话'}`);
                    lines.push(`   ID: ${hit.session.id}`);
                    lines.push(`   时间: ${formatBeijingTime(hit.session.createdAt)}`);
                    lines.push(`   排名: 词法 #${hit.lexicalRank ?? '-'} · 语义 #${hit.semanticRank ?? '-'} · 融合分 ${hit.score.toFixed(5)}`);
                    if (hit.explanation)
                        lines.push(`   解释: ${hit.explanation.summary}`);
                    if (hit.snippet)
                ...
                ctx.companion.notice('warning', `语义检索失败（原因：${error instanceof Error ? error.message : String(error)}）`);
                return {
                    kind: 'error',
        ...
        handler: async (invocation) => {
            ...
            try {
                ...
                for (const cluster of clusters.slice(0, 12)) {
                    lines.push(`${cluster.id} ${cluster.label} · ${cluster.size} 个会话 · ${formatBeijingTime(cluster.from)} ~ ${formatBeijingTime(cluster.to)}`);
        ...
        handler: async () => {
            try {
                ...
                for (const card of report.cards) {
                    const flag = card.severity === 'critical' ? '‼' : card.severity === 'watch' ? '△' : '·';
        ...
        handler: async () => {
            try {
                ...
                for (const spot of report.blindSpots) {
                    lines.push(`· 「${spot.anchor}」×${spot.searches} 次（最近 ${formatBeijingTime(spot.lastAt)}）`);
    ...
    ctx.effect(() => ctx.commands.register({
        ...
        handler: async () => {
            try {
                ...
                const lines = ['学习排序器（FTRL-Proximal 在线学习）：', ''];
                ...
                lines.push(`  训练样本 ${diagnostics.examples} 条（点击 ${diagnostics.clicks} 次），累计更新 ${diagnostics.updates} 轮`);
                lines.push(`  暖机进度 ${(diagnostics.warmup * 100).toFixed(0)}%（满 50 次更新后微调达全幅 ±30%）`);
                lines.push(`  最近训练：${diagnostics.trainedAt > 0 ? formatBeijingTime(diagnostics.trainedAt) : '（无）'}`);
                lines.push('');
                lines.push('特征权重（正 = 提高点击概率，负 = 降低；0 = 该特征被稀疏化）:');
                for (const feature of diagnostics.features) {
                    const bar = feature.weight === 0
                        ? '·'
                        : feature.weight > 0
                            ? '+'
                            : '-';
                    lines.push(`  ${bar} ${feature.name}: ${feature.weight >= 0 ? '+' : ''}${feature.weight.toFixed(4)}`);
                }
                lines.push('');
