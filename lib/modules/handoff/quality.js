 ...
 * @param options 评分参数。
 */
export function scoreHandoffQuality(sourceText, summaryText, options = {}) {
    ...
    const compressionScore = sourceChars < COMPRESSION_NEUTRAL_SOURCE_CHARS
        ? 15
        : compression >= 3
            ? 15
            : compression <= 1
                ? 0
                : ((compression - 1) / 2) * 15;
    const score = Math.round(termCoverage * 50 + (structureHits / 4) * 20 + lengthScore + compressionScore);
    const verdict = score >= 80 ? 'strong' : score >= 60 ? 'fair' : 'weak';
    return {
        score,
        verdict,
...
/** 词元长度加成：长词更可能是领域显著词。 */
function salienceMultiplier(term) {
    if (term.length >= 3)
        return 3;
    if (term.length === 2)
        return 2;
...
/** 质量报告人话摘要。 */
function summarizeQuality(verdict, score, coverage, missing, structureHits) {
    ...
    if (parts.length === 0)
        return `${head}——覆盖、结构、长度、压缩全部达标，可放心交接`;
