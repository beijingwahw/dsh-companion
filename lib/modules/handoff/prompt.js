 ...
 */
export function buildHandoffPrompt(conversationContent) {
 ...
 * @param conversationContent 格式化后的对话转录文本。
 */
export function buildHandoffPromptWithTemplate(template, conversationContent) {
 ...
 */
export function buildHandoffPromptWithFocus(conversationContent, focus) {
 ...
 * @param focus 可选的聚焦重点。
 */
export function buildMapPrompt(chunkContent, positionHint, focus) {
    return [
 ...
 */
export function buildReducePrompt(chunkSummaries, focus, templateInstruction) {
    const instruction = templateInstruction !== undefined && templateInstruction.trim().length > 0
