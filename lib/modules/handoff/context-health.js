...
export function estimateTokens(text) {
    ...
    for (let index = 0; index < text.length; index += 1) {
        ...
        if (isCjkCode(code) || isFullwidthCode(code))
            wide += 1;
        else
            narrow += 1;
    }
 ...
 */
export function computeContextHealth(turns, options = {}) {
    ...
    const avgTurnTokens = recent.length > 0
    ...
    if (turns.length >= 2 && avgTurnTokens > 0) {
        remainingTurns = Math.max(0, Math.floor((windowTokens - estimatedTokens - reservedOutputTokens) / avgTurnTokens));
    }
