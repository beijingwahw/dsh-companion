 ...
 */
export function chunkTranscript(turns, budget) {
    if (budget <= 0 || turns.length === 0)
        return [];
    const chunks = [];
    let current = [];
    let currentLength = 0;
    ...
    for (const turn of turns) {
        ...
        if (currentLength + connector + block.length > budget)
            flush();
        current.push(block);
...
export class HandoffChunkStore {
    ...
    constructor(domain) {
        this.table = domain.table('handoff-chunks');
    }
    ...
    async put(chunk, focus, summary, model) {
 ...
 */
export async function mapReduceSummarize(turns, budget, callModel, cache, focus, templateInstruction) {
    ...
    for (let index = 0; index < chunks.length; index += 1) {
        const positionHint = `第 ${index + 1}/${chunks.length} 段（约 ${Math.round(((index + 0.5) / chunks.length) * 100)}% 处）`;
