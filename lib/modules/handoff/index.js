...
import { HandoffChunkStore, mapReduceSummarize, } from './hierarchical.js';
import { LineageStore } from './lineage.js';
import { buildHandoffPromptWithFocus, buildHandoffPromptWithTemplate, } from './prompt.js';
import { MAX_TEMPLATES, MAX_TEMPLATE_CONTENT_CHARS, MAX_TEMPLATE_NAME_CHARS, TemplateStore, } from './templates.js';
import { scoreHandoffQuality } from './quality.js';
...
export function apply(ctx) {
     ...
     */
    async function generate(sessionId, templateName, focus) {
        let snapshot;
        try {
        ...
        if (formatted.length > TRANSCRIPT_CHAR_BUDGET) {
            const stores = await storesReady;
            const result = await mapReduceSummarize(turns, TRANSCRIPT_CHAR_BUDGET, callModel, stores.chunks, focus, templateContent);
            return {
    ...
    ctx.effect(() => ctx.companion.http.add('GET', '/handoff/lineage', async (_req, res, { query }) => {
    ...
    ctx.effect(() => ctx.companion.http.add('GET', '/handoff/context-health', async (_req, res, { query }) => {
